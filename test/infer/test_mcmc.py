...
class TestMALA:
    ...
    def test_normal(self):
        ...
        sigma = 1.
        data_size = 3
        normal_data = torch.distributions.normal.Normal(-1., sigma).sample((data_size,))
        ...
        true_std  = 1./torch.sqrt(torch.Tensor([factor]))
        ...
        assert np.isclose(kl, 0., atol=1e-3)
    ...
    def test_multivariate_normal(self):
        ...
        true_mean = torch.matmul(inv_additions,
                                 (torch.matmul(inv_sigma_0, mu_0) + data_size*torch.matmul(inv_sigma, normal_data.mean(dim=0))))
        ...
        assert np.isclose(kl, 0., atol=1e-2)
    ...
    def test_random_walk(self):
        ...
        def negative_log_likelihood(logit_p, y):
            ...
            T = y.shape[0]
            logp = torch.log(p)
            ll = (T - 1) * log1mp + ((y[1:] - y[:-1] + 1) / 2.).sum() * (logp - log1mp)
            return -ll
        ...
        trial_samples = sampler.run(
            (true_p + 0.2).unsqueeze(0), true_data, scale=1.
        )
        thinned_trial_samples = torch.stack(trial_samples)[::10].T
        cov = torch.cov(thinned_trial_samples)
        ...
        mala = mcmc.MALA(prior, negative_log_likelihood,
                         w=1., gradient_clipping_norm=np.inf)
        sampler = mcmc.MCMC(mala, 20_000)
        post_samples = sampler.run(thinned_trial_samples.mean(dim=1), true_data,
        ...
        assert np.isclose(np.mean(post_samples), est_p, rtol=0.05)
